...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const addToCart = (medicineId) => {
    ...
    const medicineToAdd = allMedicines.find(
        ...
        const itemHtml = `
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <small class="text-muted">Price: ₹${item.price.toFixed(
                              2
                            )}</small>
                        </div>
                        <div class="d-flex align-items-center">
                            ...
                            <button class="btn btn-sm btn-outline-danger btn-remove-item" data-id="${
                              item.id
                            }">&times;</button>
                        </div>
                    </li>
  ...
  const generateOrderSummary = () => {
    ...
    cart.forEach((item) => {
      summary += `${item.quantity} x ${item.name} (@ ₹${item.price.toFixed(
        2
      )} each)\n`;
    });
  ...
  if (medicinesListContainer) {
    ...
    // 1) Load inline JSON instantly
    const inlineDataEl = document.getElementById("inline-medicines-data");
    if (inlineDataEl) {
      try {
        allMedicines = JSON.parse(inlineDataEl.textContent);
        populateCategories(allMedicines);
        renderMedicines(allMedicines);
      } catch (err) {
        console.error("Inline JSON parse error:", err);
      }
    }

    // 2) Fetch latest data from server in background (cache-busting)
    fetch(`./medicines.json?t=${Date.now()}`)
      .then((response) => response.json())
      .then((latestData) => {
        // Only update UI if data actually changed
        if (JSON.stringify(latestData) !== JSON.stringify(allMedicines)) {
          allMedicines = latestData;
          populateCategories(allMedicines);
          renderMedicines(allMedicines);
        }
      })
                    ...
                    <div class="col-lg-4 col-md-6 mb-4">
                        <div class="card h-100 aero-card">
                            <div class="card-body d-flex flex-column">
                                ...
                                <h6 class="card-subtitle mb-2 text-primary">₹${med.price.toFixed(
                                  2
                                )}</h6>
                                <p class="card-text"><small>${
                                  med.description
                                }</small></p>
                                <p class="card-text"><span class="badge bg-light text-dark">${
                                  med.category
                                }</span></p>
  ...
  if (orderForm) {
    ...
    if (cart.length === 0) {
      ...
      let summaryHtml =
        '<h4 class="mb-3">Your Order</h4><ul class="list-group mb-3">';
      cart.forEach((item) => {
        summaryHtml += `
